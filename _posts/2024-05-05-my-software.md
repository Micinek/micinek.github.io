...
Before i had this "stack", i had to do a lot of steps to get my media, to place them in correct folders, rename them, and be able to play it on any of my devices, than i found out that this ["arr stack"](https://wiki.servarr.com/) exists and now i have fully automated workflow that will get any media based on single request from one web page that runs in my homelab.
...
Nginx is a webserver that can be used as webserver, loadbalancer and reverse proxy. I use nginx as internal reverse proxy with some plugins as certbot to obtain SSL certificates for my internal and external domains.
